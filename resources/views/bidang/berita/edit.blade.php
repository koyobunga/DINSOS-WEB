...
@section('main')
    <div class="card p-4">
          ...
          <form action="{{ url('bidang/beritas/'.$data->id) }}" method="POST" enctype="multipart/form-data">
            @method('put')
            @csrf
            <div class="mb-2">
            ...
            @if ($data->image)
                  <img src="{{ url('img/berita/'.$data->image) }}" class="img-preview img-fluid mt-2 col-sm-3 mb-3 d-block">
              @else
                  <img class="img-preview img-fluid mt-2 col-sm-3 mb-3">
              @endif

            <textarea id="elm1"  name="desc" id="" cols="30" rows="10">
              {!! $data->desc !!}
            </textarea>
